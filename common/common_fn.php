<?php
function show_article($query,$page= 1,$num= 10){	
	$conn = db_connect();
	$start = ($page-1)*$num;
	...
	$rows = $result->num_rows;
			for($i=1;$i<=$rows;$i++){
				...
				.<i class='icon-good'></i>赞({$row['fav']})<i class='icon-price-tag'></i>关键字: {$row['keywords']}</p>";
				echo "</div>";
			}
	...
	echo "<ul id='page_select'><li><a href='{$_SERVER['PHP_SELF']}'>首</a></li>";
	if($page>1){
		echo "<li><a href='".$_SERVER['PHP_SELF']."?page=".($page-1)."'>&lt&lt</a></li>";
	}else{
		echo "<li id='disabled'><span>&lt&lt</span></li>";
		echo "<li><span id='current'>1</span></li>";
	}
	if($page-2>=1){
		if($page!=3){
				echo "<li><span>...</span></li>";
				}
		echo "<li><a href='".$_SERVER['PHP_SELF']."?page=".($page-2)."'>".($page-2)."</a></li>";
		echo "<li><a href='".$_SERVER['PHP_SELF']."?page=".($page-1)."'>".($page-1)."</a></li>";
		echo "<li><span id='current'>{$page}</span></li>";
	}else if($page==2){
		echo "<li><span id='current'>2</span></li>";
	}
	...
	if($page<$total_page){
		echo "<li><a href='".$_SERVER['PHP_SELF']."?page=".($page+1)."'>&gt&gt</a></li>";
	}else{
		echo "<li id='disabled'><span>&gt&gt</span></li>";
	}
	echo "<li><a href='".$_SERVER['PHP_SELF']."?page=".($total_page)."'>尾</a></li>";
